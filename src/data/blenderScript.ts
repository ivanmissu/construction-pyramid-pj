/**
 * Blender 5.2 一键生成脚本（作为字符串导出，用户下载为 .py 后运行）
 * 严格按照 Petrie 测绘数据与连通剖面建立
 */
...
export const BLENDER_SCRIPT = `# -*- coding: utf-8 -*-
"""
================================================================================
...
def action_fcurves(action):
    ...
    if action is None:
        return []
    curves = []
    # 1) 旧式 Action（即使是新版本，部分 Action 仍会在此暴露）
    try:
        curves.extend(list(action.fcurves))
    except Exception:
        pass
    # 2) 分层 Action：layers → strips → channelbags
    try:
